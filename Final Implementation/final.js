...
var Scatterplot = function(xValue, yValue, circColour = "teal"){
    ...
    this.setupScales = function(xRange, xDomain, yRange, yDomain){
        ...
        var scales = [xAxisScale, yAxisScale];
        return scales;
    };
     ...
     */
    this.setupAxes = function(xLabel, yLabel, xScale, yScale){
     ...
     */
    this.createCircles = function(xScale, yScale, xAxisSelector, yAxisSelector){
        ...
        this.datapoints = this.svgContainer.selectAll("circle")
            ...
            .attr("cx", function(d){
                ...
                return xScale(d[xAxisSelector]);
            })
            .attr("cy", function(d){
                return yScale(d[yAxisSelector]);
            })
            // change some styling
...
function highlightBrushedCircles() {
    ...
    if (d3.event.selection != null) {
        ...
        var circles = d3.selectAll("#vis").selectAll("circle");
        ...
        var selected = circles.filter(function () {
            ...
            .each(function (d) {
                console.log(d.Player);
                var brushedPlayer = d.Player;

                var selectedPlayers = d3.selectAll("circle")
                    .filter(function (d) {
                        if (d != null && typeof d.Player != "undefined") {
                            return d.Player == brushedPlayer;
                        }

                    });
                selectedPlayers.attr("class", "brushed")
                    .style("fill", "teal");
...
function setup(){
    ...
    _vis = new Scatterplot("Salary", "Points");
    ...
    _vis2 = new Scatterplot("Salary", "Rebounds");
    ...
    //_vis2.svgContainer.append("g")
      //  .call(brush);



    loadData("NBA_2018-19_Data - Worksheet.csv", _vis2, 16);


    _vis3 = new Scatterplot("Salary", "Assists");
    _vis3.svgContainer = d3.select("#vis3");
    _vis3.width = _vis3.svgContainer.node().getBoundingClientRect().width != undefined ?
        _vis3.svgContainer.node().getBoundingClientRect().width :
        _vis3.width;
    ...
    loadData("NBA_2018-19_Data - Worksheet.csv", _vis3, 12);
}
...
function loadData(path, _visCurrent, yDomain){
    ...
    d3.csv(path).then(function(data){
        ...
        var scales = _visCurrent.setupScales([MARGINS.left, _visCurrent.width-MARGINS.left], [0, 40000000],
            [_visCurrent.height-MARGINS.bottom, MARGINS.top], [0, yDomain]);

        console.log(scales[0]);
        _visCurrent.setupAxes(_visCurrent.xValue, _visCurrent.yValue, scales[0], scales[1]);

        _visCurrent.createCircles(scales[0], scales[1]);
    });
}
